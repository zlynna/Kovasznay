...
class Plotting:
    ...
    def Saveplot(self, u_pre, v_pre, fneq_pre, Eq_res, x_train, y_train):
        # exact solution
        ...
        for i in range(1, 9):
            ...
            print('Error f_neq %d: %e' % (i, error_fneq))

        for i in range(9):
            error_eq = np.mean(Eq_res[:, i])
            print('Error Equation_res %d: %e' % (i, error_eq))
        ...
        self.plotter(fig5, ax34, Eq_res[:, 0], r'$Eq_{f_{0}}$', 'x', 'y', xx, yy)
